...
class EV_CHARGE(object):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.SOC = {agent: 20 for agent in self.agents} #{agent: random.uniform(45, 270) for agent in self.agents}
        ...
        self.target_SOC = {agent: random.uniform(0.5*self.battery_capacity, 0.8*self.battery_capacity) for agent in self.agents}
        #Randomize when EVs will depart for their first trip and time left at the depot
        self.departure_times = {agent: random.uniform(8, self.num_intervals) for agent in self.agents}
        ...
        self.safety_buffer = {agent: self.target_SOC[agent] * 1.1 for agent in self.agents}

        # self.safety_buffer = {agent: .6*self.battery_capacity for agent in self.agents}
        self.previous_action = {agent: 0 for agent in self.agents} 
        ...
        for agent in self.agents:
            self.observations[agent] = {
                ...
                "Safety_buffer": np.array([self.safety_buffer[agent]/self.battery_capacity], dtype=np.float32)}
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            if self.timestep == self.departure_times[agent]:
                if self.SOC[agent] < self.target_SOC[agent]:
                    ...
                    print(self.rewards[agent], "undercharge", self.SOC[agent])
                    
                elif self.SOC[agent] >= self.target_SOC[agent] and  self.SOC[agent] <= self.safety_buffer[agent]:
                    print("Found sweetspot", sufficient_battery_reward)
                    ...
                    self.rewards[agent] = self.safety_buffer[agent] - self.SOC[agent] 
                    print("Overcharged", self.rewards[agent])
            ...
            if self.SOC[agent] == self.battery_capacity:
                ...
                print("met", self.rewards[agent])
            
            if action > 0:
                charge_rate = action * 350
                
                if  self.SOC[agent] >= self.target_SOC[agent]:
                    overcharge_attempt_penalty[agent] = -100
            ...
            charge_rate_penalty[agent] = self.agent_actions[agent]*0
        ...
        for agent in self.agents:
            ...
            if  self.SOC[agent] < self.target_SOC[agent]:
                gap_penalty[agent] = ((self.SOC[agent]-self.target_SOC[agent])*2)*.1
        
            elif self.SOC[agent] >= self.target_SOC[agent] and self.SOC[agent]<=self.safety_buffer[agent]:
                gap_penalty[agent] = 0
                
            else:
                gap_penalty[agent] = ((self.safety_buffer[agent]- self.SOC[agent])*4)*.1
            ...
            # else:
            #     gap_penalty[agent] = 0
            if agent not in self.departed_agents:
        ...
        for agent in self.possible_agents:
            time_left_depot = (self.departure_times[agent] - self.timestep)
            self.observations[agent] = {
                "SOC": np.array([self.SOC[agent]/self.battery_capacity], dtype=np.float32),
                "Target_SOC": np.array([self.target_SOC[agent]/self.battery_capacity], dtype=np.float32),
                "Time_left_Depot": np.array([time_left_depot/self.num_intervals], dtype=np.float32),
                "Safety_buffer": np.array([self.safety_buffer[agent]/self.battery_capacity], dtype=np.float32)
                #,
                #"Available_Stations": np.array(charging_stations_available)
            }
            self.previous_action[agent] = actions[agent]
