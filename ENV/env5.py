...
def agent_reentry(
    depart_agents, EV, SOC, next_millage, target_SOC, agents, terminations
):
...
class MultiAgentEVCharge_V5(ParallelEnv):
    metadata = {"name": "Depot_env_v0"}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.charging_station_availability = {
            station: True for station in range(self.num_stations)
        }
        ...
        self.target_SOC = {
            agent: get_target_SOC(self.next_millage[agent], self.battery_capacity)
            for agent in self.agents
        }
        # Randomize when EVs will depart for their first trip and time left at the depot
        self.departure_times = {
            agent: generate_departure_time(self.timestep + 1, self.num_intervals)
            for agent in self.agents
        }
        ...
        charging_stations_available = [
            1 if value else 0 for value in self.charging_station_availability.values()
        ]
        ...
        for agent in self.agents:
            ...
            self.observations[agent] = {
                ...
                "Time_left_Depot": np.array(
                    [self.departure_times[agent]], dtype=np.float32
                ),
    ...
    def step(self, actions):
        ...
        agent_actions = {agent: 0 for agent in self.possible_agents}
        ...
        for agent in self.agents:
            ...
            if self.timestep == self.departure_times[agent]:
                ...
                if self.assigned_station[agent] != None:
                    station = self.assigned_station[
                        agent
                    ]  # Get station at which agent is currently assigned
                    self.charging_station_availability[station] = (
                        True  # Set the charging station as ava
                    )
                ...
                if self.SOC[agent] < self.target_SOC[agent]:
                    undercharge_penalty[agent] = (
                        (self.target_SOC[agent] - self.SOC[agent]) ** 2
                    ) * self.alpha
                    self.contributions[agent]["undercharge"][0] += 1
                    self.contributions[agent]["undercharge"][1] += (
                        self.target_SOC[agent] - self.SOC[agent]
                    )
                ...
                self.observations[agent] = {
                    "SOC": np.array([0], dtype=np.float32),
                    "Target_SOC": np.array([0], dtype=np.float32),
                    "Time_left_Depot": np.array([0], dtype=np.float32),
                    "Available_Stations": np.array(np.zeros(self.num_stations)),
                }
            ...
            if action > 0:
                ...
                charge_rate = 0
            agent_actions[agent] = charge_rate

        # Check if available stations can accomodate charging requests
        if len(charge_request) <= sum(
            value == True for value in self.charging_station_availability.values()
        ):
        ...
        for agent in self.agents:
            if self.assigned_station[agent] != None and agent_actions[agent] == 0:
            ...
            elif self.assigned_station[agent] != None:
                charge_rate_agent = agent_actions[agent]
                ...
                if self.SOC[agent] > self.battery_capacity:
                    overcharge_penalty[agent] = (
                        (self.SOC[agent] - self.battery_capacity) * charge_rate_agent
                    ) * self.beta
                    self.contributions[agent]["overcharge_bat"][0] += 1
                    self.contributions[agent]["overcharge_bat"][1].append(
                ...
                if self.SOC[agent] < self.target_SOC[agent]:
                    gap_penalty[agent] = (
                        (1 - (self.SOC[agent] / self.target_SOC[agent]))
                        / (1 - (self.timestep / self.departure_times[agent]))
                    ) * 50
        ...
        future_energy_window_array = np.full(
            len(self.energy_prices), fill_value=-1, dtype=np.float32
        )
        future_energy_window_array[self.timestep :] = self.energy_prices[
            :remaining_timesteps
        ]
        charging_stations_available = [
            1 if value else 0 for value in self.charging_station_availability.values()
        ...
        for agent in self.agents:
            time_left_depot = (
                self.departure_times[agent] - self.timestep
            ) / self.num_intervals
            self.observations[agent] = {
                "SOC": np.array(
                    [self.SOC[agent] / self.battery_capacity], dtype=np.float32
                ),
                "Target_SOC": np.array(
                    [self.target_SOC[agent] / self.battery_capacity], dtype=np.float32
                ),
        ...
        if (
            self.complete_charge == len(self.possible_agents)
            and not self.group_reward_received
        ):
        ...
        print(self.timestep, self.rewards, agent_actions)
    ...
    def render(self):
        ...
        for agent in self.agents:
            ...
            ev_dict = {
                "name": agent,
                "charge": round((self.SOC[agent] / self.battery_capacity) * 100, 1),
                "status": status,
                "station": assigned_station,
                "Target SOC": round(
                    (self.target_SOC[agent] / self.battery_capacity) * 100, 1
                ),
                "Departure": self.departure_times[agent],
            }
