...
"""
import numpy as np
from env4 import MultiAgentEVCharge_V4
...
if __name__  == "__main__":
    ...
    env = MultiAgentEVCharge_V4(num_agents = 4)
    obs, infos = env.reset()
...
#     actor_dims.append(dims)
    ...
    maddppg_agents = MADDPG(actor_dims, critic_dims, n_agents, n_actions_1, n_actions_2, env,
                          alpha=0.001, beta=0.01, fc1=64, fc2=64, gamma=0.99, tau = 0.01, chkpt_dir='C:/Users/luisb/EVCHARGING/tmp/maddpg_v4')
  
    memory = MultiAgentReplayBuffer(40000, critic_dims, actor_dims, n_actions_1, 
                                  n_actions_2, n_agents, batch_size=256)
    ...
    #     maddppg_agents.learn(memory, i)
        ...
        for EV in agent_names:
          ...
          overcharge_att[EV].append(env.contributions[EV]["overcharge_tar"][0])
...
# Plot data from overcharge_times
    ...
    # Plot data from overcharge_att
    plt.figure(figsize=(6, 4))
    for ev, values in overcharge_att.items():
        plt.plot(values, label=ev, marker='o')
    plt.title('Overcharge Att')
    plt.xlabel('Index')
    plt.ylabel('Values')
    plt.legend()
    plt.show()
...
# Plot data from charge_att
    plt.figure(figsize=(6, 4))
    for ev, values in charge_att.items():
      plt.plot(values, label=ev, marker='o')
    plt.title('Charge Attempt')
    plt.xlabel('Index')
    plt.ylabel('Values')
    plt.legend()
    plt.show()
...
# Plot data from complete_task
    ...
    # print("contributions", env.contributions)
