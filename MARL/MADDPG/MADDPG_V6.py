...
class MultiAgentReplayBuffer:
    ...
    def store_transition(self, raw_obs, state, action, rewards, new_raw_obs, new_state, termination):
        ...
        for agent_idx in range(self.n_agents):
            state_vec = list(raw_obs[agent_idx].values())
            state_vec_ = list(new_raw_obs[agent_idx].values())
...
class Agent:
    def __init__(self, actor_dims, critic_dims, n_actions, n_agents, agent_idx, chkpt_dir, 
        ...
        self.actor = ActorNetwork(alpha, actor_dims, fc1, fc2, n_actions,
                                        name = str(self.agent_name)+"_actor", chkpt_dir=chkpt_dir)
        
        self.critic = CriticNetwork(beta, critic_dims, fc1, fc2, n_agents, n_actions, 
                                    name = str(self.agent_name)+"_critic", chkpt_dir=chkpt_dir)

        self.target_actor = ActorNetwork(alpha, actor_dims, fc1, fc2, n_actions,
                                        name = str(self.agent_name)+"_target_actor", chkpt_dir=chkpt_dir)
        
        self.target_critic = CriticNetwork(beta, critic_dims, fc1, fc2, n_agents, n_actions, 
                                    name = str(self.agent_name)+"_target_critic", chkpt_dir=chkpt_dir)
    ...
    def learn(self, memory, episode):
      ...
      #actions agent actually took
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            critic_loss = F.mse_loss(critic_value.clone(), target.detach())
            ...
            actor_loss = -agent.critic.forward(states, mu).mean()
...
if __name__  == "__main__":
    ...
    env = SingleAgentEVCharge_V5(num_agents = 4)
    obs = env.reset()
...
#     actor_dims.append(dims)
    ...
    memory = MultiAgentReplayBuffer(1000000, critic_dims, actor_dims, n_actions, n_agents, batch_size=10000)
