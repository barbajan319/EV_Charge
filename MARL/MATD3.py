...
class ReplayBuffer():
    #Buffer of memories to sample uniformely 
    ...
    def store_transition(self, state, action, reward, state_, done):
        #Find position of first available memory 
    ...
    def sample_buffer(self, batch_size):
        ...
        actions = self.action_memory[action]
...
class Agent():
    def __init__(self, alpha, beta, input_dims, tau, env,
                gamma = 0.99, update_actor_interval = 2, warmup = 10000, 
                n_actions = 1, max_size = 1000000, layer1_size = 8, layer2_size = 16,
    ...
    def choose_action(self, observation):
        ...
        mu_prime = mu +T.tensor(np.random.normal(scale=self.noise), dtype=T.float)
    ...
    def learn(self):
        ...
        rewards = T.tensor(reward, dtype = T.float)
        done = T.tensor(done)
        state = T.tensor(state, dtype = T.float)
        state_ = T.tensor(new_state, dtype = T.float)
